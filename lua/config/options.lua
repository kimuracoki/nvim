...
-- キーシーケンスの待ち時間
-- ターミナルモードのjkマッピングを削除したので、ターミナルではjにラグなし
-- 挿入モードのjkは使えるように短めの値に設定
opt.timeoutlen = 300   -- 挿入モードのjkでノーマルモード復帰用（300ms以内にjkと打てばOK）
opt.ttimeoutlen = 0    -- ターミナルモードでは待ち時間なし（jkマッピングがないので）
