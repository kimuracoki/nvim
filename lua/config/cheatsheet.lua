...
M.sections = {
  {
    ...
    keys = {
      ...
      { "SPC fs", "シンボル検索（ファイル内）" },
      { "SPC ft", "TODO コメント検索" },
      { "SPC fd", "このファイルと比べる相手を選んで差分表示（任意の 2 ファイル）" },
      { "do / dp", "差分表示の中で、相手の変更を取り込む / 相手へ送る（diffget / diffput）" },
      { "SPC fD", "差分表示を閉じる" },
      { "SPC sw", "シンボル検索（全体）" },
  ...
  },
  {
    ...
    keys = {
      ...
      { "SPC xw", "ワークスペースの診断" },
      { "SPC xq", "Quickfix 一覧" },
      { "SPC xy", "カーソル行の診断メッセージをコピー" },
      { "SPC xY", "カーソル行の診断を位置つき（path:行:列）でコピー" },
    },
  },
  {
    title = "Git",
    keys = {
      ...
      { "SPC gl", "コミットグラフ" },
      { "SPC go", "Octo メニュー" },
      { "[c / ]c", "前 / 次の変更ハンクへ（差分タブでは前 / 次の差分へ）", cover = { "[c", "]c" } },
  ...
  },
  {
    ...
    keys = {
      ...
      { "Tab / S-Tab", "補完候補を次 / 前へ・スニペットの次 / 前のプレースホルダへ", cover = { "<Tab>", "<S-Tab>" } },
      { "CR", "補完を確定（Enter を押すまで挿入されない）" },
      { "Down/Up, C-n / C-p", "補完候補を選ぶ（確定はしない）" },
      { "C-Space", "補完を表示 / ドキュメントを開閉" },
      { "C-e", "補完を閉じる" },
